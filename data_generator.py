	...
	while True:
		...
		for batch_iter in range(0, steps_per_epoch*args.batch_size, args.batch_size):
			...
			for item_idx,item_iter in enumerate(range(batch_iter,batch_iter+args.batch_size)):
				...
				# load pos item
				file_path = args.feature_path + D7id_to_path[Tid_to_D7id[Sid_to_Tid[songs[item_idx].decode('UTF-8')]]].replace('.mp3','.npy')
				...
				for neg_iter in range(args.N_negs):
					...
					file_path = args.feature_path + D7id_to_path[Tid_to_D7id[Sid_to_Tid[songs[neg_idx].decode('UTF-8')]]].replace('.mp3','.npy')
