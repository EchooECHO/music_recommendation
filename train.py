...
def main(args):
	...
	# load label data
	sorted_coo_train, sorted_coo_valid, songs, user_to_item_train, user_to_item_valid, all_items, D7id_to_path, Tid_to_D7id, Sid_to_Tid, item_to_user_train,item_to_user_valid, train_idx, valid_idx, test_idx = load_label(args)
	len_sparse = len(sorted_coo_train)

	# load valid data
	x_valid, y_valid = load_valid(args, sorted_coo_valid, songs, user_to_item_valid, all_items, D7id_to_path, Tid_to_D7id, Sid_to_Tid, train_idx, valid_idx, test_idx, item_to_user_valid)

	# make weight directory
	steps_per_epoch = int(100)#int(len_sparse/args.batch_size/10) # randomly chosen epoch
	weight_name = './models/model_%s_%d_%.2f/weights.{epoch:02d}-{val_loss:.2f}.h5' % (args.model,args.N_negs,args.margin)
	#weight_name='./models/model/model_basic_20_0.20/weights.h5'
	...
	# run model
	model.fit_generator(generator=train_generator(args, sorted_coo_train, songs, user_to_item_train, all_items, D7id_to_path, Tid_to_D7id, Sid_to_Tid, steps_per_epoch, train_idx, valid_idx, test_idx, item_to_user_train),
					steps_per_epoch=steps_per_epoch,
					workers=args.workers,
					use_multiprocessing=False, #True to False
					max_queue_size=1,
					epochs=args.epochs,
					verbose=1,
					callbacks=callbacks,
...
if __name__ == '__main__':
	...
	parser.add_argument('--N', type=int, default=1, help='save weight every N epochs')
	...
	parser.add_argument('--workers', type=int, default=40, help='the number of generators to be used') # 40 to 1
	...
	parser.add_argument('--feature-path', type=str, default='/Users/ECHO/Documents/songtest/deep-content-user-master/data/msd/save_path_100/', help='mel-spectrogram path')
	...
	parser.add_argument('--num-user', type=int, default=1000, help='the number of users')
	parser.add_argument('--num-song', type=int, default=100, help='the number of items')
